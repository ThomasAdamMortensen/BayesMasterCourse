...
Here we can see the autocorrelation between samples is very "sticky" for the samples using the identity covariance matrix.

```{r}
acf(PoisRWMDraws$X1)
acf(PoisRWMDraws3$X1)
...
fit = stan(file = "Poisreg.stan", data = Poisreg_dat)
```
